...
This analysis focuses on the LAX departures dataset. The dataset contains information about the number of departures to different airports from Tom Bradley terminal, LAX. We will explore the top destinations and categorize them as domestic or international flights.  
...
#### **What are the most popular destinations for outgoing flights from Tom Bradley terminal, LAX?**

To determine the most popular destinations for outgoing flights from Tom Bradley terminal, LAX, we have analyzed the dataset and identified the top 10 destinations based on the number of departures.
...
departures_dd <- read.csv("LAX_departures_dd.csv")
...
<section style="border-top: 1px solid black;"></section>  
The bar chart above displays the top 10 most popular airport destinations based on the number of daily departures from Tom Bradley terminal, LAX. The airports are ranked by the number of departures, with the highest number at the top.

As we can see from the chart, the most popular destination for outgoing flights from Tom Bradley terminal, LAX is **`r top_airports$airport_name[1]`**, with a total of **`r top_airports$top_airports[1]`** departures.
...
The bar chart above displays the top 10 most popular airlines based on the number of departures. It provides insights into the prominence of different airlines at Tom Bradley terminal, LAX.  
...
Based on the chart, the busiest time of day for departures from Tom Bradley terminal, LAX is **`r paste0(peak_hours$timeslot[which.max(peak_hours$total)], ":00")`**. The number of flights departing at this time is: **`r peak_hours$total[which.max(peak_hours$total)]`**. During this period, there is a significant surge in departures, indicating a high level of activity at the airport.
...
directory <- file.path("..", "Raw_Data", "LAX")
...
Additionally, we analyzed the average delay time and the proportion of delayed flights at Tom Bradley terminal, LAX.

<section style="border-top: 1px solid black;"></section>


We determined the proportion of flights that experienced delays. The analysis revealed that approximately **`r round(delays[[2]], 3) * 100`%** of flights departing from Tom Bradley terminal, LAX experienced some form of delay. This metric is crucial for understanding the reliability and punctuality of flight operations at the airport. It highlights the importance of implementing strategies and measures to minimize delays, enhance passenger satisfaction, and ensure smooth travel experiences.
...
By examining the average delay time and the proportion of delayed flights, Tom Bradley terminal, LAX can assess its performance in terms of timeliness and take appropriate measures to improve operational efficiency and minimize disruptions for passengers.
